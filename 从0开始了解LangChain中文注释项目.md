...
更核心的一项改动是新增源码与中文注释的结构一致性校验。这个项目的主要价值来自 `langchain_code/` 和 `code_comment/` 的一一对照关系，所以测试会检查每个源码 `.py` 文件是否都有对应中文 `.md` 注释，并限制中文注释目录中无来源文件的出现。包级概览文档和少量专题概览文档作为显式例外保留，避免把有价值的学习资料误判为孤儿文件。
